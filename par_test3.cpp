...
const char *config_string = R"(
    ...
    // 原始全局矩阵 CSR（nnz=12）
    // row_ptrs = {0,4,6,9,12}
    // col_idx = {0,1,2,3, 1,3, 0,1,2, 0,2,3}
    // vals    = {1,2,5,7, 4,9, 10,11,13, 12,15,14}
    ...
    // // 创建分布句柄并设置 partition_offsets
    // // two ranks each own 2 rows -> offsets = {0,2,4}
    // AMGX_distribution_handle dist;
    // AMGX_CHECK(AMGX_distribution_create(&dist, cfg));
    // int partition_offsets[3] = {0, local_n, global_n};
    // AMGX_CHECK(AMGX_distribution_set_partition_data(
    //     dist,
    //     AMGX_DIST_PARTITION_OFFSETS,
    //     partition_offsets));

    // 上传分布式矩阵
    // std::vector<int64_t> col_idx_64(col_idx.begin(), col_idx.end());

    // 上传矩阵时使用int64_t的col_idx
